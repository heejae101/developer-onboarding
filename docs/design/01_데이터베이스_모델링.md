...
### 2.10 GIS_SAMPLE (GIS 실습 샘플)

| 컬럼명 | 타입 | 제약조건 | 코멘트 |
|:---|:---|:---|:---|
| `id` | BIGINT | PK, AUTO_INCREMENT | 샘플 ID |
| `title` | VARCHAR(200) | NOT NULL | 샘플 제목 |
| `description` | TEXT | NULL | 설명 |
| `geojson` | JSON | NOT NULL | GeoJSON 본문 |
| `created_at` | TIMESTAMP | NOT NULL, DEFAULT NOW() | 생성일시 |
| `updated_at` | TIMESTAMP | NOT NULL, DEFAULT NOW() | 수정일시 |

---

### 2.11 KNOWLEDGE_POST (지식 공유 게시글)

| 컬럼명 | 타입 | 제약조건 | 코멘트 |
|:---|:---|:---|:---|
| `id` | BIGINT | PK, AUTO_INCREMENT | 게시글 ID |
| `title` | VARCHAR(200) | NOT NULL | 제목 |
| `content_md` | TEXT | NOT NULL | Markdown 본문 |
| `tags` | VARCHAR(200) | NULL | 태그 CSV |
| `author_id` | BIGINT | FK(USER.id), NOT NULL | 작성자 ID |
| `created_at` | TIMESTAMP | NOT NULL, DEFAULT NOW() | 생성일시 |
| `updated_at` | TIMESTAMP | NOT NULL, DEFAULT NOW() | 수정일시 |
